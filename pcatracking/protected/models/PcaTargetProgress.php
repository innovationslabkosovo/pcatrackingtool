...
class PcaTargetProgress extends CActiveRecord
{
    ...
    //        'condition' => $condition,
            'order' => $this->tableName().'.target_progress_id ASC',
        );  
	...
	public function rules()
	{
		...
		return array(
			array('target_progress_id, pca_id', 'required'),
			array('pca_target_progress_id, pca_id, total, current, shortfall', 'numerical', 'integerOnly'=>true),
			array('received_date, active', 'safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('pca_id, total, current, shortfall', 'safe', 'on'=>'search'),
		);
	}

	public function primaryKey()
	{
		return array('pca_target_progress_id');
		# code...
	}
	/**
	...
	public function relations()
	{
		...
		return array(
			'tpTarget' => array(self::BELONGS_TO, 'TargetProgress', array('target_progress_id')),
